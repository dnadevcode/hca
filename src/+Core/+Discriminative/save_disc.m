...
outFold = fullfile(fileparts(fileparts(hcaSets.kymofolder{1})),['distinct',hcaSets.timestamp,'.csv']);
  ...
  fid = fopen(outFold, 'w');
  fprintf(fid, '"Barcode_name","First_match", "First_match_CC","len_first_match", "Second_match", "Second_match_CC","len_second_match","Is_distinct"\n');
  for i=1:length(is_distinct)
      if is_distinct(i)==1 || plotNonDistinct
          ...
          fprintf(fid,'%s,%s,%4.3f,%4.3f,%s,%4.3f,%4.3f,%4.3f\n',fname,first_match,first_match_cc,len_first_match,second_match,second_match_cc,len_second_match,is_distinct(i));
      end
  end
  fclose(fid);
