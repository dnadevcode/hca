function [  ] = plot_comparison_vs_theory_all(comparisonStruct,theoryStruct,barcodeGen,selectedIndices,savetxt)
...
%         end
        ...
        if ~isempty(a)
            plot([fitPositions(1:a-1) nan fitPositions(a:end)], 0.1*i+[((expBar(1:a-1)-m2)/s2)*s1+m1 nan ((expBar(a:end)-m2)/s2)*s1+m1])
        else
            plot(fitPositions, 0.1*i+((expBar-m2)/s2) *s1+m1)
        end   
...
%         if switched == 0
            legends{i+1}=strcat(['$\hat C_' num2str(ii) '$=' num2str(maxcoef(ii,1),'%0.2f')]) ;
%         else
...
%         end
    ...
    legend(legends,'Interpreter','latex','Location','southoutside')
end
