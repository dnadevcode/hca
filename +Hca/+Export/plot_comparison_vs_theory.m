function [  ] = plot_comparison_vs_theory(comparisonStruct,theoryStruct,barcodeGen,selectedIndices,savetxt)
    ...
    for i=1:length(selectedIndices)
        ...
        niceName = strrep(strrep(theoryStruct{comparisonStruct{ii}.idx}.filename,'_',' '),'.txt','');
        ...
        if switched == 0
            legend({strcat(['$\hat C_' num2str(ii) '$=' num2str(maxcoef(ii,1),'%0.2f')]),niceName},'Interpreter','latex');
        else
